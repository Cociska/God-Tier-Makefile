# ==================================================
# Compilers & Flags
# ==================================================

CC              = epiclang
CFLAGS          = -Wall -Wextra -Werror
CSFMLFLAGS      = -lcsfml-window -lcsfml-graphics -lcsfml-audio -lcsfml-system
MATHSFLAGS      = -lm
CRITERION_FLAGS = -lcriterion --coverage

# ==================================================
# Colors
# ==================================================

C_RESET   = \033[0m
C_BOLD    = \033[1m

C_GREEN   = \033[32m
C_BLUE    = \033[34m
C_CYAN    = \033[36m
C_YELLOW  = \033[93m
C_PURPLE  = \033[35m
C_ORANGE  = \033[38;5;208m

# ==================================================
# Paths & Files
# ==================================================

BROWSER    = firefox

SCRIPTS_PATH    =  ~/Makefile/scripts
SRC_PATH   = src/
TESTS_PATH = tests/
UTILS_PATH = utils/

SRC = 	main.c

SRC_TESTS = $(TESTS_PATH)tests.c

OBJ       = $(SRC:.c=.o)

NAME      = project_name
TEST_BIN  = test_runner

# ==================================================
# Build rules
# ==================================================

all: $(NAME)

$(NAME): $(OBJ)
	@$(CC) -o $(NAME) $(OBJ)
	@echo "$(C_GREEN)Compiled (^_^)$(C_RESET)"

%.o: %.c
	@echo "$(C_BLUE)Compiling $<$(C_RESET)"
	@$(CC) $(CFLAGS) -c $< -o $@

auto_build:
	@clear
	@echo "Recommendation : open in a background terminal :P"
	@$(SCRIPTS_PATH)/auto_build.sh

# ==================================================
# Run & Debug
# ==================================================

run:
	@clear
	all
	@./$(NAME)
	@echo "$(C_GREEN)Program executed correctly :<$(C_RESET)"

docker:
	@clear
	@echo "$(C_BLUE)Starting Epitech Docker...$(C_RESET)"
	@docker run -it -v ./:/usr/app epitechcontent/epitest-docker:latest

debug:
	@clear
	@gcc -g3 $(SRC) -o $(NAME)_debug $(CFLAGS)
	@echo "$(C_CYAN)>>> Starting GDB...$(C_RESET)"
	@gdb -ex "break main" -ex "run" ./$(NAME)_debug
	@rm -f $(NAME)_debug

leaks:
	@clear
	all
	@valgrind --leak-check=full ./$(NAME)

# ==================================================
# Tests & Coverage
# ==================================================

tests:
	@clear
	@gcc $(CFLAGS) $(SRC) $(SRC_TESTS) -o $(TEST_BIN) $(CRITERION_FLAGS)
	@echo "$(C_YELLOW)Tests compiled :9$(C_RESET)"
	@./$(TEST_BIN)
	@echo "$(C_GREEN)Tests executed ^^;$(C_RESET)"

coverage: tests
	@lcov -c -d . -o coverage.info --ignore-errors mismatch
	@echo "$(C_CYAN)Coverage data collected o_o$(C_RESET)"
	@lcov -r coverage.info "*/tests/*" -o coverage_filtered.info
	@echo "$(C_CYAN)Test files removed from coverage report ^.-$(C_RESET)"
	@genhtml coverage_filtered.info -o html_report
	@echo "$(C_CYAN)HTML coverage report generated ^^;$(C_RESET)"
	@$(BROWSER) html_report/index.html
	@echo "$(C_GREEN)Coverage report opened in browser :)$(C_RESET)"

# ==================================================
# Cleaning
# ==================================================

clean:
	@clear
	@rm -f $(OBJ)
	@echo "$(C_BLUE)Object files removed :D$(C_RESET)"
	@rm -f *.gcda *.gcno *.gcov
	@echo "$(C_BLUE)Test artifacts cleaned ;)$(C_RESET)"
	@rm -rf html_report coverage.info coverage_filtered.info
	@echo "$(C_BLUE)Coverage files deleted :P$(C_RESET)"

fclean: clean
	@rm -f $(NAME)
	@echo "$(C_CYAN)Binary removed <_<$(C_RESET)"

re: 	fclean all
	@echo "$(C_BOLD)$(C_YELLOW)Project rebuilt successfully \\o/$(C_RESET)"

# ==================================================
# Git
# ==================================================

commit:
	@clear
	@$(SCRIPTS_PATH)/commit.sh

restore:
	@clear
	@$(SCRIPTS_PATH)/restore.sh

branch:
	@clear
	@$(SCRIPTS_PATH)/branch.sh

git_log:
	@clear
	@$(SCRIPTS_PATH)/git_log.sh

# ==================================================
# Info rules
# ==================================================

count:
	@clear
	@$(SCRIPTS_PATH)/count.sh

stats:
	@clear
	@$(SCRIPTS_PATH)/stats.sh

# ==================================================
# Fun & focus
# ==================================================

star_wars:
	@$(SCRIPTS_PATH)/star_wars.sh

pomodoro:
	@clear
	@$(SCRIPTS_PATH)/pomodoro.sh

weather:
	@clear
	@$(SCRIPTS_PATH)/weather.sh

coffee:
	@clear
	@$(SCRIPTS_PATH)/coffee.sh

joke:
	@$(SCRIPTS_PATH)/joke.sh

radio:
	@$(SCRIPTS_PATH)/radio.sh

# ==================================================
# AI Assistant
# ==================================================

api:
	@$(SCRIPTS_PATH)/api.sh

claude:
	@$(SCRIPTS_PATH)/claude.py $(MSG)
	
claude_fix:
	@$(SCRIPTS_PATH)/claude_fix.sh

# ==================================================
# Update & Maintenance
# ==================================================

update:
	@clear
	@$(SCRIPTS_PATH)/update.sh
	
uninstall:
	@$(SCRIPTS_PATH)/uninstall.sh

# ==================================================
# Help & menu
# ==================================================

menu:
	@clear
	@$(SCRIPTS_PATH)/menu.sh

help:
	@echo ""
	@echo "$(C_BOLD)$(C_CYAN)⚡ God Tier Makefile — targets disponibles$(C_RESET)"
	@echo ""
	@echo "$(C_BOLD)$(C_GREEN)Build & Run$(C_RESET)"
	@echo "  $(C_GREEN)make$(C_RESET)              Compile le projet"
	@echo "  $(C_GREEN)make run$(C_RESET)          Compile + exécute"
	@echo "  $(C_GREEN)make auto_build$(C_RESET)   Watch mode (rebuild à chaque save)"
	@echo "  $(C_GREEN)make debug$(C_RESET)        Build debug + lance GDB"
	@echo "  $(C_GREEN)make docker$(C_RESET)       Lance l'env Epitech Docker"
	@echo "  $(C_GREEN)make leaks$(C_RESET)        Valgrind (leaks)"
	@echo ""
	@echo "$(C_BOLD)$(C_YELLOW)Tests & Coverage$(C_RESET)"
	@echo "  $(C_YELLOW)make tests$(C_RESET)        Lance les tests (Criterion)"
	@echo "  $(C_YELLOW)make coverage$(C_RESET)     Génère + ouvre le report HTML (lcov)"
	@echo ""
	@echo "$(C_BOLD)$(C_BLUE)Cleaning$(C_RESET)"
	@echo "  $(C_BLUE)make clean$(C_RESET)        Supprime .o + artefacts tests/coverage"
	@echo "  $(C_BLUE)make fclean$(C_RESET)       Clean + supprime le binaire"
	@echo "  $(C_BLUE)make re$(C_RESET)           Rebuild complet"
	@echo ""
	@echo "$(C_BOLD)$(C_ORANGE)Git (fzf)$(C_RESET)"
	@echo "  $(C_ORANGE)make commit$(C_RESET)       Commit/push via script"
	@echo "  $(C_ORANGE)make branch$(C_RESET)       Switch/crée une branche (fzf)"
	@echo "  $(C_ORANGE)make restore$(C_RESET)      Restore fichiers (fzf)"
	@echo "  $(C_ORANGE)make git_log$(C_RESET)      Git log interactif (fzf)"
	@echo ""
	@echo "$(C_BOLD)$(C_CYAN)Info$(C_RESET)"
	@echo "  $(C_CYAN)make count$(C_RESET)        count lines"
	@echo "  $(C_CYAN)make stats$(C_RESET)        project stats"
	@echo ""
	@echo "$(C_BOLD)$(C_PURPLE)Focus & Fun$(C_RESET)"
	@echo "  $(C_PURPLE)make pomodoro$(C_RESET)     Pomodoro"
	@echo "  $(C_PURPLE)make coffee$(C_RESET)       Pause café"
	@echo "  $(C_PURPLE)make weather$(C_RESET)      Météo (CITY=$(CITY))"
	@echo "  $(C_PURPLE)make joke$(C_RESET)         Blague dev"
	@echo "  $(C_PURPLE)make radio$(C_RESET)        Radio (RADIO_URL)"
	@echo "  $(C_PURPLE)make star_wars$(C_RESET)    Star Wars ASCII"
	@echo ""
	@echo "$(C_BOLD)$(C_ORANGE)AI$(C_RESET)"
	@echo "  $(C_ORANGE)make api$(C_RESET)         setup api"
	@echo "  $(C_ORANGE)make claude$(C_RESET)      ask claude (MSG=\"...\")"
	@echo "  $(C_ORANGE)make claude_fix$(C_RESET)  ai fix logs"
	@echo ""
	@echo "$(C_BOLD)$(C_YELLOW)Maintenance$(C_RESET)"
	@echo "  $(C_YELLOW)make update$(C_RESET)      update scripts"
	@echo "  $(C_YELLOW)make uninstall$(C_RESET)   uninstall tools"
	@echo ""
	@echo "$(C_BOLD)$(C_CYAN)UI$(C_RESET)"
	@echo "  $(C_CYAN)make menu$(C_RESET)         Menu interactif (fzf)"
	@echo "  $(C_CYAN)make signature$(C_RESET)    Affiche la signature"
	@echo "  $(C_CYAN)make help$(C_RESET)         Affiche ce help"
	@echo ""
	@$(MAKE) --no-print-directory signature
	@echo ""


signature:
	@$(SCRIPTS_PATH)/signature.sh

.PHONY: \
	all run auto_build docker debug leaks \
	tests coverage \
	clean fclean re \
	commit restore branch git_log \
	count stats \
	api claude claude_fix \
	update \
	menu help signature \
	star_wars pomodoro coffee weather joke radio

